...
// ============================================================
// NEW: Generic “ticket export” that includes STUDENT + TABLE
// ============================================================

function buildTicketHTML(title, train, trainFields, rows, rowColumns) {
    ...
    <div class="kv">
      ${(trainFields || [])
        ...
        <div class="kv-row">
          ...
          <div class="v">${escHtml(train?.[f.key])}</div>
        </div>
      `
        )
        .join("")}
  ...
  const tableBlock = `
    ...
    `
        : `<div class="muted">No reservations found.</div>`
    }
  `;
...
// - blank line
// - Enrollments table
function buildTicketCSV(train, trainFields, rows, rowColumns) {
  const trainLines = (trainFields || []).map(
    (f) => `${safeCsv(f.label)},${safeCsv(train?.[f.key])}`
  );
...
/**
 * Export ticket data to CSV (student details + enrollments)
 *
 ...
 */
export function exportTicketToCSV(filename, train, rows, config) {
...
/**
 * Export ticket data to PDF (student details + enrollments)
 *
 * @param {string} title
 * @param {object} train
 * @param {Array<object>} rows
 ...
 *  - rowColumns: [{key,label}]
 */
export function exportTicketToPDF(title, train, rows, config) {
